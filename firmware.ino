...
const int MOTOR_STEPS_PER_REV[] = {400, 400, 400, 400, 400, 400};
...
// joint limits & “rest” positions
int JOINT_LIMIT_MIN[] = {-170, -42, -89, -180, -105, -180};
int JOINT_LIMIT_MAX[] = {170, 90, 52, 180, 105, 180};
const int REST_MOTOR_STEPS[] = {7555, 2333, 4944, 8960, 2295, 4000};

enum State { STATE_TRAJ, STATE_ERR };
...
const int LIMIT_SWITCH_HIGH[] = {1, 1, 1, 1, 1, 1};
const int CAL_DIR[] = {-1, -1, 1, -1, -1, 1};
const int CAL_SPEED = 500;
const int CAL_SPEED_MULT[] = {1, 1, 1, 2, 1, 1};
int ENC_RANGE_STEPS[NUM_JOINTS];

float JOINT_MAX_SPEED[] = {60.0, 60.0, 60.0, 60.0, 60.0, 60.0}; // °/s
float JOINT_MAX_ACCEL[] = {30.0, 30.0, 30.0, 30.0, 30.0, 30.0}; // °/s²
...
bool safeRun(AccelStepper &s) { return estop_pressed ? false : s.run(); }
bool safeRunSpeed(AccelStepper &s) {
  return estop_pressed ? false : s.runSpeed();
}

// read encoder → motor steps
...
String JointPosToString(double *pos) {
  String out;
  for (int i = 0; i < NUM_JOINTS; ++i) {
    out += JOINT_NAMES[i];
    out += String(pos[i], 6);
...
String JointVelToString(double *vel) {
  String out;
  for (int i = 0; i < NUM_JOINTS; ++i) {
    out += JOINT_NAMES[i];
    out += String(vel[i], 6);
...
void setupSteppers() {
  for (int i = 0; i < NUM_JOINTS; i++) {
    ...
    stepperJoints[i].setPinsInverted(false, false, false);
...
void MoveVelocity(String inData) {
  ...
  for (int i = 0; i < NUM_JOINTS; i++) {
    ...
    cmdJointVel[i] *= MOTOR_STEPS_PER_DEG[i];
...
bool doCalibrationRoutine(String &outputMsg, int calJoints[NUM_JOINTS],
  ...
  while (!AtPosition(REST_MOTOR_STEPS, curMotorSteps, 5)) {
    ...
    for (int i = 0; i < NUM_JOINTS; ++i) {
      if (!calJoints[i]) {
        curMotorSteps[i] = REST_MOTOR_STEPS[i];
      }
    }
    MoveTo(REST_MOTOR_STEPS, curMotorSteps);
...
void updateMotorVelocities(int *motorSteps, int *lastMotorSteps,
  ...
  for (int i = 0; i < NUM_JOINTS; i++) {
    ...
    if (abs(stepperJoints[i].speed() / MOTOR_STEPS_PER_DEG[i]) < 5) {
      // NB! trying to fix artifacts at low velocity
      if (abs(motorSteps[i] - checksteps[i]) > 0) {
        lastVelocity[i] = stepperJoints[i].speed() / MOTOR_STEPS_PER_DEG[i];
      ...
      if (abs(motorSteps[i] - checksteps[i]) > 0) {
        ...
        lastVelocity[i] = ENC_DIR[i] * (motorSteps[i] - lastMotorSteps[i]) /
                          MOTOR_STEPS_PER_DEG[i] / (delta / 1000000.0);
        lastMotorSteps[i] = motorSteps[i];
